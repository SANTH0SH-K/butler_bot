...
class ButlerRobotTask2(Node):
    def __init__(self):
        super().__init__('butler_robot_task2')
        ...
        self.get_logger().info("Butler Robot Task 2 initialized and ready to receive orders")
    ...
    def execute_delivery(self):
        for goal in self.goals:
            ...
            if not self.wait_for_confirmation():
                self.get_logger().info(f'No confirmation received at {goal}. Returning to home.')
                self.publish_status(f'No confirmation received at {goal}. Returning to home.')
                
                # Go directly home if no confirmation
                self.navigate_to('home')
